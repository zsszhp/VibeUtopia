...
### A/B 回测验证任务执行中
- Date: 2026-05-15
- Context: 开始执行阶段 3 A/B 回测验证（验收标准：A 组比 B 组命中率提升≥15%）
- Category: 测试方法
- Instructions:
  - **测试脚本**: `tests/phase3_ab_test.py`（386 行）
  - **测试案例**: 使用 cases/paperwork/目录下的回测案例
  - **测试方法**:
    - A 组（实验组）：人生故事驱动 Agent（基于文案推断人格特质 + 故事增强标记）
    - B 组（对照组）：传统属性标签 Agent（仅基础人格属性）
    - 每组使用相同的风险评估流程，唯一变量是人格是否包含人生故事增强
  - **验收标准**: A 组准确率 - B 组准确率 ≥ 15%
  - **评估指标**:
    - 风险等级判断准确率（red/orange/yellow/green）
    - 两组准确率差异
  - **执行状态**: 准备运行（2026-05-15）
