...
### 参考资源与测试文件恢复（2026-05-16 更新，不可删除）
- Date: 2026-05-16
- Context: 用户要求恢复被删除的tests测试文件，下载references中所有提到的项目和论文，编写详细技术分析
- Category: 代码结构 | 环境配置
- Instructions:
  - **tests/video/ 恢复**: 从git历史提交bac02697恢复4个测试案例（ai/fight/mhy/moon）的视频(.mp4)、音频(.mp3)和封面(.jpg)文件，共12个文件
  - **Git LFS 配置**: 仅跟踪tests/video/下的.mp4/.mp3/.jpg文件（.gitattributes配置），Gitee不支持LFS需用GIT_LFS_SKIP_PUSH=1跳过
  - **references/projects/ 本地保留**: 25+个开源项目源码本地保留，不上传git（.gitignore忽略），按6个分类组织：
    - core/: BettaFish, DeepSearchAgent-Demo, MiroFish, TrendRadar, ex-skill（5个核心参考项目）
    - agent-simulation/: AgentSociety, Concordia, OASIS, SoMe, llm-social-network, awesome-llm-human-simulation（6个智能体仿真框架）
    - memory-rag/: Mem0, MemGPT, GraphRAG, nano-graphrag, LightRAG（5个记忆RAG系统）
    - content-safety/: Sensitive-lexicon, content-moderation-deep-learning（2个内容安全项目）
    - multimodal/: faster-whisper, PaddleOCR, PySceneDetect（3个多模态分析项目）
    - llm-frameworks/: LiteLLM, LangGraph, AutoGen, CrewAI（4个LLM工具框架）
    - VideoRAG/（根目录，视频RAG参考项目）
  - **references/papers/ 本地保留**: 25+篇PDF论文本地保留，不上传git（.gitignore忽略）
  - **references/analysis/ 技术分析文档（26个，已上传git）**: 每个项目都有详细的深度技术分析文档，包含项目概述、核心架构、关键技术实现、对VibeUtopia的参考价值、精华与糟粕
    - core: BettaFish, DeepSearchAgent, MiroFish, TrendRadar, ex-skill, StanfordGA
    - agent-simulation: AgentSociety, Concordia, OASIS, SoMe, llm-social-network, awesome-llm-human-simulation
    - memory-rag: Mem0, MemGPT, GraphRAG, nano-graphrag, LightRAG
    - content-safety: Sensitive-lexicon, content-moderation-deep-learning
    - multimodal: faster-whisper, PaddleOCR, PySceneDetect
    - llm-frameworks: LiteLLM, LangGraph, AutoGen, CrewAI
  - **references/decisions.md**: 技术选型决策总表
  - **项目架构整理**: docs编号统一(00-28)、tests脚本分类(scripts/子目录)、配置去重(删除2个死文件)、安全修复(移除明文API Key)
  - **提交版本**: commit b6fbfd74
  - **重要**: tests/video/和references/projects/中的文件不可删除；references/projects/和references/papers/不上传git但本地保留

### CLAUDE.md 项目指令文件（2026-05-16 创建）
- Date: 2026-05-16
- Context: 用户要求设置默认 skill 调用规则
- Category: 代码结构 | 环境配置
- Instructions:
  - **位置**: 项目根目录 `CLAUDE.md`
  - **作用**: 控制 Claude Code 在本项目中的默认行为，每次会话自动加载
  - **强制 Git 工作流**: 动手前后必须 pull/push，commit message 用中文
  - **默认 Skill 调用规则**:
    - 会话开始 → `using-superpowers`
    - 创建功能/修改行为 → `brainstorming` → `writing-plans`
    - 代码搜索/定位/小修改 → `cavecrew`（省 60% token）
