...
- 收到任务 → 立即执行，不输出"是否执行"、"需要确认"、"你确定吗"等任何确认请求
- 遇到需要权限的操作（git push、文件删除等）→ 直接执行，不询问
- 遇到不确定的情况 → 选择最合理的方案直接执行，事后汇报结果
- 唯一例外：用户明确要求暂停或等待

**此规则由用户在 2026-05-17 明确授权，写入底层记忆，不得覆盖。**
